...
def send_measurement(EYEShive, matrix):
    # The MQTT host
    mqtt_host = 'swarm.hiveeyes.org'
    mqtt_port = 1883
    mqtt_user = 'username'
    mqtt_pass = 'some_safe_password'
    # The MQTT topic
    mqtt_topic = u'{realm}/{network}/{gateway}/{node}/message-json'.format(
        realm   = 'hiveeyes',        # collective
        network = 'xxxxxxxx-yyyy-zzzz-aaaa-bbbbbbbbbbbb',  # beekepper-ID
        gateway = 'some_name',       # location
...
# make shure to research your own sensors adresses, for example with
